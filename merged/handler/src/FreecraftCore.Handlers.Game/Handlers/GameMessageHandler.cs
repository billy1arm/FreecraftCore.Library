...
{
	/// <summary>
	/// Base contract for all game message handlers.
	/// </summary>
	/// <typeparam name="TGamePayloadType">The payload this handler is responsible for.</typeparam>
	public abstract class GameMessageHandler<TGamePayloadType> : INetworkMessagePipelineListener<GameNetworkMessageContext, NetworkOperationCode, IGamePacketHeader, GamePacketPayload>
		where TGamePayloadType : GamePacketPayload 
	{
		public NetworkMessageContextState RecievePipelineMessage([NotNull] GameNetworkMessageContext input, NetworkMessageContextState currentState)
		{
		...
		/// <returns></returns>
		protected abstract NetworkMessageContextState RecieveMessage([NotNull] GameNetworkMessageContext context, [NotNull] TGamePayloadType stronglyTypedPayload);
	}
}
