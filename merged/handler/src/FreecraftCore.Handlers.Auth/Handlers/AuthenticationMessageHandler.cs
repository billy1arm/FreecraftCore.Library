...
{
	/// <summary>
	/// Base contract for all authentication message handlers.
	/// </summary>
	/// <typeparam name="TAuthenticationPayloadType">The payload this handler is responsible for.</typeparam>
	public abstract class AuthenticationMessageHandler<TAuthenticationPayloadType> : INetworkMessagePipelineListener<AuthenticationNetworkMessageContext, AuthOperationCode, IAuthenticationPacketHeader, AuthenticationPayload>
		where TAuthenticationPayloadType : AuthenticationPayload
	{
		public NetworkMessageContextState RecievePipelineMessage([NotNull] AuthenticationNetworkMessageContext input, NetworkMessageContextState currentState)
		{
			...
			return RecieveMessage(input, stronglyTypedPayload) | currentState;
		}
		...
		/// <returns></returns>
		protected abstract NetworkMessageContextState RecieveMessage([NotNull] AuthenticationNetworkMessageContext context, [NotNull] TAuthenticationPayloadType stronglyTypedPayload);
	}
}
