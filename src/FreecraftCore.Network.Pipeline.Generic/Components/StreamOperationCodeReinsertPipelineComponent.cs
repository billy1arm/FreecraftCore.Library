		...
		where TPayloadType : IMessageVerifyable
	{
		...
		public Task<IWireStreamReaderStrategyAsync> RecievePipelineMessageAsync([NotNull] IWireStreamReaderStrategyAsync input, TNetworkContextBuilderType currentState)
		{
			...
			if (currentState.GameHeader == null)
				throw new InvalidOperationException($"Failed to read the {nameof(NetworkOperationCode)} from the stream. There was no {nameof(IGamePacketHeader)} in the {nameof(IGameNetworkMessageContextBuilder)} available.");
